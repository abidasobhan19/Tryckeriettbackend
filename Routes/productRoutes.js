...
const ALLOWED_PACKAGING_TYPES = new Set(["none", "plastic_bag", "cardboard", "custom"]);
...
function normalizePackagingType(value) {
  ...
  if (!ALLOWED_PACKAGING_TYPES.has(normalized)) {
    throw new Error(`Packaging type "${stringifyCellValue(value)}" is not supported.`);
  }
...
function normalizeUploadLabelMethod(value) {
  ...
  if (normalized === "digitalprintingfullcolor" || normalized === "digitalprintfullcolor" || normalized === "digital") {
    return { key: "digitalPrintingFullColor", label: "Digital Printing Full Color" };
  }

  if (normalized === "screenprinting" || normalized === "screenprint" || normalized === "screen") {
    return { key: "screenPrinting", label: "Screen Printing" };
  }

  if (normalized === "transferprinting" || normalized === "transferprint" || normalized === "transfer") {
...
function buildUploadVariants(row) {
  ...
  [
    ...
    { field: "variantNumber", aliases: ["variantNumber", "variantSku", "variantId", "variantProductNumber"] },
    {
      field: "imageUrls",
      aliases: ["variantImageUrls", "variantImageUrl", "variantAssetUrls", "variantAssetUrl", "variantImages", "variantImage"],
    },
  ...
  return Array.from(variantsByIndex.entries())
    ...
    .reduce((items, [, variant]) => {
      const productColor = stringifyCellValue(variant.productColor) || undefined;
      const variantNumber = stringifyCellValue(variant.variantNumber) || undefined;
...
async function resolveCategoryUploadReference(value, categoryCache) {
  ...
  const matchedCategory = await Category.findOne({ name: exactMatcher }).select("_id name").lean();
...
function buildUploadRowPreview(rawRow) {
  ...
  return {
    productName: stringifyCellValue(pickUploadValue(row, ["productName", "name"])) || undefined,
    productNumber:
      stringifyCellValue(pickUploadValue(row, ["productNumber", "productId", "sku", "id"])) || undefined,
  };
}
...
async function buildBulkUploadPayload(rawRow, seenProductNumbers, supplierCache, categoryCache) {
  const row = buildNormalizedUploadRow(rawRow);
  const productName = stringifyCellValue(pickUploadValue(row, ["productName", "name"]));
  const productNumber = stringifyCellValue(pickUploadValue(row, ["productNumber", "productId", "sku", "id"]));
  const categoryInput = stringifyCellValue(pickUploadValue(row, ["category", "categoryName"]));
  const supplierInput = pickUploadValue(row, ["supplierName", "supplier", "supplierCompany", "companyName"]);
  ...
  if (seenProductNumbers.has(duplicateCheckKey)) {
    throw new Error(`Product number "${productNumber}" is duplicated in the spreadsheet.`);
  }
  seenProductNumbers.add(duplicateCheckKey);

  const [{ supplier, supplierName }, { category, categoryId }] = await Promise.all([
    resolveSupplierUploadReference(supplierInput, supplierCache),
    resolveCategoryUploadReference(categoryInput, categoryCache),
  ]);

  const productColor = stringifyCellValue(pickUploadValue(row, ["productColor", "color"])) || undefined;
  const additionalInformation =
    stringifyCellValue(pickUploadValue(row, ["additionalInformation", "additionalInfo"])) || undefined;
  const description = stringifyCellValue(pickUploadValue(row, ["description"])) || undefined;
  ...
  const packagingType = normalizePackagingType(pickUploadValue(row, ["packagingType", "packaging"]));
  ...
  const assets = buildUploadAssets(pickUploadValue(row, ["imageUrls", "imageUrl", "assetUrls", "assetUrl", "images", "image"]));
...
productRouter.post("/bulk-upload", async (req, res) => {
  ...
  if (rows.length === 0) {
    return res.status(400).json({ message: "Upload file is empty or missing data rows." });
  }

  if (rows.length > BULK_UPLOAD_LIMIT) {
    return res
      .status(400)
      .json({ message: `Bulk upload supports up to ${BULK_UPLOAD_LIMIT} rows at a time.` });
  }
  ...
  for (let index = 0; index < rows.length; index += 1) {
    ...
    try {
      const payload = await buildBulkUploadPayload(rows[index], seenProductNumbers, supplierCache, categoryCache);
